...
def update():
    ...
    for pid, name, gender, category, price, image, stock in PRODUCTS_DATA:
        cursor.execute(
            """INSERT INTO products (id, name, gender, category, price, image, stock)
               ...
               ON DUPLICATE KEY UPDATE
                   ...
                   image = VALUES(image),
                   stock = VALUES(stock)""",
            (pid, name, gender, category, price, image, stock),
        )

    connection.commit()
    cursor.close()
    connection.close()
    print("Successfully updated 16 products with HD fashion photography!")
